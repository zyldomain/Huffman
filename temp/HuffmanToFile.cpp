...
{
	//打开被压缩的文件 
 	FILE *fIn = fopen(sourceFileName,"rb");
 	//打开解压后的文件的位置 
 	...
 	{
 		//printf("%d  ",++count);
 		road = (unsigned char)code;
 		int n = 0;
 		while(n < 8)
 		{
 			n++;
 			unsigned char dir = (unsigned char)(road%2+'0');
 			road /= 2;
 			printf("%c",dir);
 			//如果该路径为0，则向左走 
 			if(dir == '0')
			...
			{
				//将该字符输出到解压文件中 
				fputc((unsigned char)p->character,fOut);
				//重置索引节点 
				p = tree;
			}
				//如果该路径为0，则向左走 
...
//将生成的编码文件变成二进制 
void transToBinary(char *targetFileName,char *finalFileName)
{
	//打开被编码的文件 
 	FILE *fIn = fopen(targetFileName,"rw");
 	//打开解压后的文件
 	FILE *fOut = fopen(finalFileName,"wb");
 	unsigned char character;
 	int ch;
	while(!feof(fIn))
	{
		...
		for(int i = 0;i<8;i++)
		{
			ch = (unsigned char)(fgetc(fIn)-48);
			//printf("%d",ch);
			character += ch*pow(2,i);
		}
		//将字符的编码写到要压缩的文件中 
		printf("%d ",character);
		fputc(((unsigned char)character),fOut);
	}
	fclose(fOut);
	fclose(fIn);
...
int main(int fileNum,char **fileName)
{
	...
	char targetFileName[200] = "temp.txt";
	...
	{
		//源文件   压缩文件 
		strcpy(finalFileName,fileName[2]);
	...
	//	printf("%c",-70);
		...
		transToBinary(targetFileName,finalFileName);
	}
	//对文件进行解压 
	else if(fileNum == 4)
	{
